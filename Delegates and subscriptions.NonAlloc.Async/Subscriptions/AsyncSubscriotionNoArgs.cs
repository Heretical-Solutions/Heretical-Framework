...
{
	public class AsyncSubscriptionNoArgs
		: INonAllocSubscription,
		  INonAllocSubscriptionContext<IAsyncInvokableNoArgs>,
		  ICleanuppable,
		  IDisposable
	{
		private readonly IAsyncInvokableNoArgs @delegate;
		...
		public AsyncSubscriptionNoArgs(
			IAsyncInvokableNoArgs @delegate,
			ILogger logger)
		{
		...
		public IAsyncInvokableNoArgs Delegate
		{
			get => @delegate;
		}
			...
			INonAllocSubscribable publisher)
		{
			...
			if (publisher is not IAsyncPublisherNoArgs)
			{
				logger?.LogError(
					GetType(),
					$"INVALID PUBLISHER: EXPECTED {nameof(IAsyncPublisherNoArgs)} : {this.GetHashCode()}");
