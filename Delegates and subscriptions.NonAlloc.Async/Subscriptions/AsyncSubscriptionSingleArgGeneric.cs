...
{
	public class AsyncSubscriptionSingleArgGeneric<TValue>
		: INonAllocSubscription,
		  INonAllocSubscriptionContext<IAsyncInvokableSingleArg>,
		  INonAllocSubscriptionContext<IAsyncInvokableSingleArgGeneric<TValue>>,
		  ICleanuppable,
		  IDisposable
	{
		private readonly IAsyncInvokableSingleArgGeneric<TValue> @delegate;

		private readonly ILogger logger;

		private INonAllocSubscribable publisher;

		public AsyncSubscriptionSingleArgGeneric(
			IAsyncInvokableSingleArgGeneric<TValue> @delegate,
			ILogger logger)
		{
		...
		IAsyncInvokableSingleArg
			INonAllocSubscriptionContext<IAsyncInvokableSingleArg>.Delegate
		{
			get => (IAsyncInvokableSingleArg)@delegate;
		}

		IAsyncInvokableSingleArgGeneric<TValue> 
			INonAllocSubscriptionContext<IAsyncInvokableSingleArgGeneric<TValue>>.Delegate
		{
			get => @delegate;
		}
			...
			INonAllocSubscribable publisher)
		{
			...
			if (publisher is not IAsyncPublisherSingleArg
				&& publisher is not IAsyncPublisherSingleArgGeneric<TValue>)
			{
				logger?.LogError(
					GetType(),
					$"INVALID PUBLISHER: EXPECTED {nameof(IAsyncPublisherSingleArg)} OR {nameof(IAsyncPublisherSingleArgGeneric<TValue>)} : {this.GetHashCode()}");
