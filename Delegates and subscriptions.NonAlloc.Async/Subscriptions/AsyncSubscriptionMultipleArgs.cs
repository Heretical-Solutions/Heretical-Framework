...
{
	public class AsyncSubscriptionMultipleArgs
		: INonAllocSubscription,
		  INonAllocSubscriptionContext<IAsyncInvokableMultipleArgs>,
		  ICleanuppable,
		  IDisposable
	{
		private readonly IAsyncInvokableMultipleArgs @delegate;
		...
		public AsyncSubscriptionMultipleArgs(
			IAsyncInvokableMultipleArgs @delegate,
			ILogger logger)
		{
		...
		public IAsyncInvokableMultipleArgs Delegate
		{
			get => @delegate;
		}
			...
			INonAllocSubscribable publisher)
		{
			...
			if (publisher is not IAsyncPublisherMultipleArgs)
			{
				logger?.LogError(
					GetType(),
					$"INVALID PUBLISHER: EXPECTED {nameof(IAsyncPublisherMultipleArgs)} : {this.GetHashCode()}");
