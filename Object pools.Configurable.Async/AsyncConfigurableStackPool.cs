...
{
	public class AsyncConfigurableStackPool<T>
		: IAsyncPool<T>,
		  IAsyncResizable,
		  ...
		  IDisposable
	{
		...
		private readonly AsyncConfigurableStackPoolFactory factory;

		private int capacity;

		public AsyncConfigurableStackPool(
			Stack<T> pool,
			IAsyncAllocationCommand<T> allocationCommand,
			AsyncConfigurableStackPoolFactory factory)
		{
			this.pool = pool;
		...
		public async Task Resize(
			...
			AsyncExecutionContext asyncContext)
		{
			capacity = await factory.ResizeAsyncConfigurableStackPool(
				pool,
				capacity,
				allocationCommand,
				asyncContext);
